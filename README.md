...
core, so `cpu` there carries `usage_idle`/`usage_active` only; no disk inodes;
`load*` is zero on Windows, which has no load-average concept. `net`/`disk` counters are emitted cumulative — take the
