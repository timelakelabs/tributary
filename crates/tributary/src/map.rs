...
pub fn map_line(src: &Source, line: &str) -> Result<(Record, i64), MapError> {
    ...
    let parsed: BTreeMap<String, serde_json::Value> = match src.parser {
        // DockerJson, Cri, Journald and Winlog are JSON here: an upstream step
        // (the docker/CRI reassembler / the journald reader / the winlog reader)
        // has already turned the wire form into a JSON object whose keys are
        // the parsed map.
        Parser::Json | Parser::DockerJson | Parser::Cri | Parser::Journald | Parser::Winlog => {
